...
class CompanyController {
  ...
  static createCompany(companyData) {
    return new Promise((resolve, reject) => {
      const { company_code, company_name, company_address, company_logo, currency, company_phone, company_email } = companyData;

      db.query(
        'INSERT INTO companies (company_code, company_name, company_address, company_logo, currency, company_phone, company_email) VALUES (?, ?, ?, ?, ?, ?, ?)',
        [company_code, company_name, company_address, company_logo, currency, company_phone, company_email],
        (err, result) => {
          if (err) {
  ...
  static updateCompanyByCode(companyCode, companyData) {
    return new Promise((resolve, reject) => {
      const { company_name, company_address, company_logo, currency, company_phone, company_email } = companyData;

      let query = 'UPDATE companies SET company_name = ?, company_address = ?, currency = ?, company_phone = ?, company_email = ?';
      let params = [company_name, company_address, currency, company_phone, company_email];
