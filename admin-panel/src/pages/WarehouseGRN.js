...
export default function WarehouseGRN() {
        ...
        try {
            ...
            if (data.success && data.purchase_orders?.length > 0) {
                ...
                if (data.purchase_orders.length === 1) {
                    navigate(`/warehouse/grn/${data.purchase_orders[0].po_number}`);
                }
            } else {
    ...
    const loadGRNHistory = async (page = 1, search = '') => {
        try {
            let url;
            if (selectedPO) {
                url = `${BASE_URL}/api/admin/grn/grn-history/${selectedPO}?company_code=${company_code}&page=${page}&limit=10&search=${search}`;
            } else {
                url = `${BASE_URL}/api/admin/grn/history?company_code=${company_code}&page=${page}&limit=10&search=${search}`;
            }
            ...
            console.error('Error loading GRN history:', err);
        }
    };
    ...
    return (
        <Container fluid className="warehouse-grn-container">
            ...
            <Breadcrumb className="mb-4">
                <Breadcrumb.Item href="/warehouse">Warehouse</Breadcrumb.Item>
                <Breadcrumb.Item active>GRN Processing</Breadcrumb.Item>
                {selectedPO && (
                    <Breadcrumb.Item active>{selectedPO}</Breadcrumb.Item>
                )}
            </Breadcrumb>
            ...
            <Row>
                {/* Search PO Section */}
                <Col md={selectedPO ? 8 : 12}>
                    <Card className="mb-4">
                        <Card.Body>
                            <Form onSubmit={handleSearchPO} autoComplete="off">
                                <Row className="g-3 align-items-end">
                                    <Col lg={selectedPO ? 6 : 4} md={6} style={{ position: 'relative' }}>
                                        <Form.Group>
                                                    ...
                                                    {poResults.map(po => (
                                                        <div
                                                            ...
                                                            onMouseDown={() => {
                                                                ...
                                                                setShowPODropdown(false);
                                                                navigate(`/warehouse/grn/${po.po_number}`);
                                                            }}
                                                        >
                    ...
                    <Col md={4}>
                        <Card>
                            <Card.Header>
                                <Row>
                                    <Col>
                                        <h6>GRN History for PO</h6>
                                    </Col>
                                    <Col xs="auto">
                                        <Button
                                            variant="outline-secondary"
                                            size="sm"
                                            onClick={() => loadGRNHistory(1, '')}
                                        >
                                            Refresh
                                        </Button>
                                    </Col>
                            ...
                            <Card.Body>
                                {grnHistory.length > 0 ? (
                                    <>
                                        {grnHistory.map((grn) => (
                                            <Card key={grn.grn_id} className="mb-2 grn-history-card">
                                                <Card.Body className="py-2">
                                                    <div className="d-flex justify-content-between">
                                                        <div>
                                                            <strong>{grn.grn_id}</strong>
                                                            <div className="small text-muted">
                                                                {new Date(grn.received_date).toLocaleDateString()}
                                                            </div>
                                                        </div>
                                                        <div className="text-end">
                                                            <Badge bg={grn.status === 'completed' ? 'success' : 'warning'}>
                                                                {grn.status}
                                                            </Badge>
                                                            <div className="small">
                                                                {grn.total_qty} items
                                                    ...
                                                    <Button
                                                        ...
                                                        size="sm"
                                                        className="p-0 mt-1"
                                                        onClick={() => navigate(`/warehouse/grn-details/${grn.grn_id}`)}
                                                    >
                                                        View Details
                                                    </Button>
                                                </Card.Body>
                                        ...
                                        <Pagination className="mt-3 justify-content-center">
                                            <Pagination.Prev 
                                                ...
                                                onClick={() => loadGRNHistory(historyPage - 1, historySearch)}
                                            />
                                            <Pagination.Item active>{historyPage}</Pagination.Item>
                                            <Pagination.Next 
                                    ...
                                    <div className="text-center text-muted py-3">
                                        No GRN history found for this PO
                                    </div>
                                )}
                            </Card.Body>
                        </Card>
